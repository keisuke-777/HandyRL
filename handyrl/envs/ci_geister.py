...
class Conv2dHead(nn.Module):
    def __init__(self, input_shape, filters, output_filters):
        ...
        self.conv1 = nn.Conv2d(
            input_shape[0], filters, kernel_size=3, stride=1, padding=1, bias=False
        )
...
class ScalarHead(nn.Module):
    def __init__(self, input_shape, filters, outputs):
        ...
        self.bn = nn.BatchNorm2d(filters)
        self.fc = nn.Linear(
            input_shape[1] * input_shape[2] * filters, outputs, bias=False
        )
...
class GeisterNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(
            input_channels, filters, kernel_size=3, stride=1, padding=1, bias=False
        )
...
class Environment(BaseEnvironment):
    ...
    def __str__(self):
        ...
        for i in range(6):
            s += (
                self.X[i]
                + " "
                + " ".join([self.P[_piece(self.board[i, j])] for j in range(6)])
                + "\n"
            )
    ...
    def diff_info(self, player):
        ...
        if len(self.record) == 0:
            if self.turn_count > -2:
                info["set"] = (
                    self.layouts[played_color] if color == played_color else -1
                )
        else:
    ...
    def observation(self, player=None):
        ...
        # colortype2piece(color, t) = color * 2 + t
        ...
        # スカラー
        # [ターンプレイヤ(先手なら1),ターンプレイヤの視点かどうか, 残っている駒数]
        # ex) 残っている駒数：駒数が3 = 0,0,1,0
        s = np.array(
            [
...
# 上位のディレクトリからテストの際に呼んでもらう関数
# ここから実行すると上位のインポート(from ..environment import BaseEnvironment)でエラー吐く
def test_cigeister():
    e = Environment()
    for _ in range(1):
        e.reset()
