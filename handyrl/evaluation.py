...
def eval_process_mp_child(
    agents, critic, env_args, index, in_queue, out_queue, seed, show=False
):
    ...
    while True:
        ...
        if isinstance(list(agent_map.values())[0], NetworkAgent):
            outcome = exec_network_match(
                env, agent_map, critic, show=show, game_args=game_args
            )
        else:
...
def evaluate_mp(
    env, agents, critic, env_args, args_patterns, num_process, num_games, seed
):
    ...
    for pat_idx, args in args_patterns.items():
        for i in range(num_games):
            if len(agents) == 2:
                ...
                tmp_pat_idx, agent_ids = (
                    pat_idx,
                    random.sample(list(range(len(agents))), len(agents)),
                )
    ...
    for p, r_map in enumerate(result_map):
        ...
        for pat_idx, results in r_map.items():
            print(
                pat_idx,
                {k: results[k] for k in sorted(results.keys(), reverse=True)},
                wp_func(results),
            )
        print(
            "total",
            {
                k: total_results[p][k]
                for k in sorted(total_results[p].keys(), reverse=True)
            },
...
def eval_main(args, argv):
    ...
    evaluate_mp(
        env, agents, critic, env_args, {"default": {}}, num_process, num_games, seed
    )
